...
proc create_root_design { parentCell } {
  ...
  set block_name uart_top
  set block_cell_name uart_top_0
  if { [catch {set uart_top_0 [create_bd_cell -type module -reference $block_name $block_cell_name] } errmsg] } {
     catch {common::send_gid_msg -ssname BD::TCL -id 2095 -severity "ERROR" "Unable to add referenced block <$block_name>. Please add the files for ${block_name}'s definition into the project."}
     return 1
   } elseif { $uart_top_0 eq "" } {
     catch {common::send_gid_msg -ssname BD::TCL -id 2096 -severity "ERROR" "Unable to referenced block <$block_name>. Please add the files for ${block_name}'s definition into the project."}
     return 1
   }
  
  # Create port connections
  connect_bd_net -net Debounce_Switch_0_o_Switch [get_bd_pins Debounce_Switch_0/o_Switch] [get_bd_pins uart_top_0/iRst]
  connect_bd_net -net iClk_0_1 [get_bd_ports iClk] [get_bd_pins Debounce_Switch_0/i_Clk] [get_bd_pins uart_top_0/iClk]
