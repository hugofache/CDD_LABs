module wrap_adder #(
  parameter   ADDER_WIDTH = 64,    // width of the internal adder
...
//    .oCarry(wCarryOut)
//  );
    carry_select_adder #( .ADDER_WIDTH(ADDER_WIDTH), .BLOCK_SIZE(BLOCK_SIZE) ) 
    CSelA   (
        .iA( rCnt[2*ADDER_WIDTH:ADDER_WIDTH+1] ), 
        .iB( rCnt[ADDER_WIDTH:1] ),
        .iCarry( rCnt[0] ),
...
//    carry_select_lookahead_adder #( .ADDER_WIDTH(ADDER_WIDTH) ) 
//    CSelLAA   (
//        .iA( rCnt[2*ADDER_WIDTH:ADDER_WIDTH+1] ), 
//        .iB( rCnt[ADDER_WIDTH:1] ),
//        .iCarry( rCnt[0] ),
